...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QHBoxLayout, QGroupBox, QDialog, QVBoxLayout, \
    QGridLayout, QTableWidget, QTableWidgetItem, QLineEdit, QLabel
...
class App(QDialog):
    ...
    def __init__(self, folder):
        ...
        self.label_list = ["Age", "Genotype", "Sex", 'Setup Time', 'Number', 'Condition', 'Condition Time',
                           'Alert Days', 'Condition Start', 'Hatch Time', 'Position']
        self.read_df()
        self.current_vial = []
        self.initUI()
    ...
    def createInput(self):
        ...
        delete_button.clicked.connect(self.delete_vial)
        buttonLayout.addWidget(hatch_button)
        buttonLayout.addWidget(flip_button)
        buttonLayout.addWidget(condition_button)
        buttonLayout.addWidget(transfer_button)
        buttonLayout.addWidget(delete_button)
        textboxLayout.addWidget(Genotype_label)
        textboxLayout.addWidget(self.Genotype_box)
        textboxLayout.addWidget(Sex_label)
        textboxLayout.addWidget(self.Sex_box)
        textboxLayout.addWidget(Number_label)
        textboxLayout.addWidget(self.Number_box)
        inputLayout.addLayout(buttonLayout)
        inputLayout.addLayout(textboxLayout)
            ...
            if self.fly_sheet['Position'].isin([self.current_vial]).any():
                ...
                self.fly_sheet = self.fly_sheet.append(
                    {'Position': self.current_vial, 'Setup Time': setuptime_str, 'Hatch Time': setuptime_str,
                     'Alert Days': '2', 'Sex': self.Sex_box.text(), 'Genotype': self.Genotype_box.text(),
    ...
    def show_info(self):
        ...
        if self.transfer_to != []:
            ...
            temp_dict['Setup Time'] = datetime.now().strftime("%m/%d/%Y, %H:%M")
    ...
    def delete_vial(self):
        self.fly_sheet = self.fly_sheet[self.fly_sheet.Position != self.current_vial]
        self.fly_sheet.to_pickle(self.folder + 'fly_sheet.pkl')
        self.refresh()
